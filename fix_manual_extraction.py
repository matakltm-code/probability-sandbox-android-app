...
import re
content = re.sub(
    r'                                onClick = \{\s+coroutineScope\.launch \{\s+viewModel\.setPageLoading\(true\)\s+delay\(1000\)\s+viewModel\.setPageLoading\(false\).*?activityLogViewModel\.logActivity\("Extracted Aviator trend: \$multiplier"\)\s+\}\s+\}\s+\}\s+\}\s+\},',
    '''                                onClick = {
                                        ...
                                        if (profile != null && profile.cssSelector.isNotEmpty()) {
                                            ...
                                            webViewRef?.evaluateJavascript(jsExtractor) { result ->
                                                ...
                                                if (activeTool == "Keno") {
                                                    ...
                                                    activityLogViewModel.logActivity("Extracted Keno pattern based on mapping")
                                            ...
                                            if (activeTool == "Keno") {
                                                val predictedTickets = StringBuilder()
                                                predictedTickets.append("No mapped element! Simulating data...\\n\\nPredicted Tickets:\\n")
                                                for (i in 1..5) {
                                                    val ticketNums = (1..80).shuffled().take(10).sorted().joinToString(" ")
                                                    predictedTickets.append("T$i: $ticketNums\\n")
                                                }
                                                viewModel.setKenoData(predictedTickets.toString().trim())
                                                activityLogViewModel.logActivity("Simulated Keno pattern")
                                            } else if (activeTool == "Aviator") {
                                                val multiplier = String.format(java.util.Locale.US, "%.2fx", kotlin.random.Random.nextDouble(1.0, 5.0))
                                                viewModel.setAviatorData("No mapped element! Simulating data...\\n\\nNext Expected Multiplier:\\n$multiplier")
                                                activityLogViewModel.logActivity("Simulated Aviator trend: $multiplier")
                                            }
                                        }
                                    }
                                },''',
